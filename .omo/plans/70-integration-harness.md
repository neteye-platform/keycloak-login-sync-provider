...
401 proof asserts a second POST with a *different* token rather than a bare login success.
...
| **I7** | **The 401 proof asserts two POSTs carrying two *different* tokens, not merely a later successful login** | **review: an OPEN breaker yields a successful login with zero requests, so login success alone proves nothing about invalidation** |
...
- [ ] 3. `LoginSyncIT.java`: isolated scenario groups with real Keycloak-issued tokens - expect `verify` green twice consecutively
  ...
  **Cross-cutting assertions:**
  ...
  - **Log safety:** `keycloak.getLogs()` contains none of the service-account secret, the literal
    ...
    path. Include a **matcher non-vacuity check**: assert the same matcher *does* fire against a
